...
import { nextTick, computed, inject, shallowRef, onBeforeUnmount, type PropType } from 'vue'
...
const self_disabled = computed(() => {
  return props.disabled || formItemContext?.disabled.value
})
// 判断当前富文本编辑器是否禁用
nextTick(() => {
  if (self_disabled.value && editorRef.value) {
...
const handleFocus = (editor: IDomEditor) => {
  ...
  props.onFocus && props.onFocus(editor)
}
